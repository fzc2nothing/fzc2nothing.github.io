...
### 409.Longest Palindrome
Use an array to store the appearing times of each character. And notice that, in Palindrome array the character are symmetric to the middle line. So there can be only one character has odd number and all of other character must have 
